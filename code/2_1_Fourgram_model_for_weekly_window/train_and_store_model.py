...
series_pickle_path = "../../data/intermediate/series_with_new_cleaner_multiple_process.pickle"

# if not os.path.isfile(series_pickle_path):
...
pickle_in = open(series_pickle_path, "rb")
...
def multiple_processing_func(input_date):
    ...
    global processed_news_series
    if input_date < np.datetime64('2018-06-08'):
        return
    else:
        model = FourGramModel(processed_news_series, input_date)
        try:
            model()
...
if __name__ == '__main__':
    ...
    p.map_async(multiple_processing_func,
                [np.datetime64(key).astype('datetime64[D]') for key in processed_news_series.index])
    p.close()
    p.join()
    end = time.time()
    ...
    pickle_out = open("../../data/intermediate/dict_of_models.pickle", "wb")
